...
// @require       https://rawgit.com/arphen/arphen.user.js/master/libs/video.js?ver=aaa
// @grant         none
// ==/UserScript==
...
function hilite(){
	...
	$('div.mlist dl dd').each(function(){
		...
		if(s.indexOf("主演")>=0){
			s = s.replace("主演：", "").replace(/\//g, " ").replace(/\|/g, " ").replace(/，/g, " ").replace(/\s+/g, " ");
			var arr = s.split(" ");
